...
const Homepage = () => {
  ...
  const addToCart = (product) => {
    const updatedCart = [...cartItems, product]; // 🛠 Thêm sản phẩm mới vào giỏ hàng
    setCartItems(updatedCart); // 🛠 Cập nhật state của giỏ hàng
    ...
    <div className="app" style={{ overflowX: "hidden" }}>
      ...
      {cartOpen && (
        <div className="cart-dropdown" ref={cartRef}>
          <h4>Có <span>{cartItems.length}</span> sản phẩm trong giỏ hàng</h4>
          {cartItems.length === 0 ? (
            <p>Giỏ hàng trống</p>
          ) : (
            <ul>
